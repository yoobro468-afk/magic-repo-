...
class gdClone(GoogleDriveHelper):
    ...
    def user_setting(self):
        if self.listener.upDest.startswith('mtp:') or self.listener.link.startswith('mtp:'):
            ...
            self.use_sa = False
        elif self.listener.upDest.startswith('sa:') or self.listener.user_dict.get('use_sa'):
    ...
    def clone(self):
        ...
        try:
            ...
            if mime_type == self.G_DRIVE_DIR_MIME_TYPE:
                ...
                file = self._copyFile(meta.get('id'), self.listener.upDest, self.listener.newname)
    ...
    def _cloneFolder(self, folder_name, folder_id, dest_id):
        ...
        for file in files:
            ...
            elif not file.get('name').lower().endswith(tuple(self.listener.extensionFilter)):
                self.total_files += 1
            ...
            if err.resp.get('content-type', '').startswith('application/json'):
                ...
                if reason == 'cannotCopyFile':
                    ...
                    if self.sa_count >= self.sa_number:
                        LOGGER.info('Reached maximum number of service accounts switching, which is %s', self.sa_count)
                        raise err
