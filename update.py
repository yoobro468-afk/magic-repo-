...
if (UPSTREAM_REPO := environ.get('UPSTREAM_REPO')) and (UPSTREAM_BRANCH := environ.get('UPSTREAM_BRANCH')):
    ...
    log_info(f'Using repo: {UPSTREAM_REPO}')
    log_info(f'Branch: {UPSTREAM_BRANCH}')

    # Removed git fetch/reset to avoid authentication crash
    log_info('Auto update disabled to prevent Git authentication errors.')
